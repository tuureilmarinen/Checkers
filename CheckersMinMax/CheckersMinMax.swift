...
class CheckersMinMax: CheckersPlayer {
    ...
    private func provideMoveWithMinMaxAlphaBeta(_ state: GameState) -> GameState? {
        _ = minMaxWithAlphaBeta(state: state, depth: self.initialSearchDepth, alpha: -.infinity, beta: .infinity)
    ...
    private func minMaxWithAlphaBeta (state: GameState, depth: Int, alpha:Double, beta:Double) -> Double {
        var alpha=alpha
        var beta=beta
        let children = state.children
        
        if knownValues[state] != nil && (guessDepth[state]!<goodEnoughSearchDepth || knownValues[state]!.magnitude==Double.infinity){
        ...
        if depth==0 {
            let r = Double(state.whitePieces.nonzeroBitCount)/Double(state.blackPieces.nonzeroBitCount)
            return r > 1 ? r : (1.0/r) * -1
        }
        ...
        if state.whiteTurn { // white turn -> maximizing
            ...
            for child in children {
                let childValue = minMaxWithAlphaBeta(state: child, depth:depth-1, alpha:alpha, beta:beta)
